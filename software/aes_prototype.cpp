// simple_aes.cpp : Defines the entry point for the console application.
//
...
#define AES_ROUND  2      // for 128 bit key size
...
void AES_ECB_decrypt(const uint8_t*, uint8_t*, uint8_t*, const uint32_t);
...
int main()
{
	...
	uint8_t expected_output[] = { 0x29, 0xC3, 0x50, 0x5F, 0x57, 0x14, 0x20, 0xF6, 0x40, 0x22, 0x99, 0xB3, 0x1A, 0x02, 0xD7, 0x3A};
	...
	printf("\n");
	*
	if (0 == memcmp((char*)expected_output, (char*)buffer, 16))
	{
		printf("SUCCESS!\n");
	}
	else
...
void AES_ECB_encrypt(const uint8_t* input, uint8_t* key, uint8_t* output, const uint32_t length)
{
	/*
...
void aes_encrypt()
{
	...
	printf("=======Pre whitening State==========\n");
	printState();
	// run this loop for n-1 rounds; 9 roundsfor AES-128
	for (round = 1; round < AES_ROUND; round++)
	{
		sub_byte();  printf("=======Round %d: Sub Byte==========\n",round);  printState();
		shift_row(); printf("=======Round %d: Shift Row==========\n", round); printState();
		mix_column(); printf("======Round %d: Mix Column==========\n", round); printState();
		add_roundKey(round); printf("=======Round %d: Key Addition==========\n", round); printState();
	}
...
For each bit in y, x is multiplied by power of 2, correspondingly with the position of bit in y
*/
